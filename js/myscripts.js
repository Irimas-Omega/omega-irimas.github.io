...
$(document).ready(function(){
	$.getJSON("jsons/people.json", function(data){
		...
		for (var i=0; i<data.alumni.length; i++){
			...
			row_pic_back = '<div class="flip-card-back img-thumbnail"><br><p>' + data.alumni[i].position + '</p><p>' + data.alumni[i].dates ;
			// row_thesis = data.alumni[i].thesis;
            // if (row_thesis != ''){
            //     row_pic_back = row_pic_back + '<br><i>' + row_thesis + '</i>';
            // }
			row_pic_back = row_pic_back + '</p></div>'
            row_pic = '<div class="col-md-2" align="center"><div class="flip-card"><div class="flip-card-inner">' + row_pic_front + row_pic_back + '</div></div>'
